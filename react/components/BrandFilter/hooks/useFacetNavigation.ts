...
import { newFacetPathName, getMainSearches } from '../utils'
import {
...
const removeElementAtIndex = (strArray: any, index: any) =>
  strArray.filter((_: any, i: any) => i !== index)

const upsert = (array: any, item: any) => {
  const foundItemIndex = array.findIndex(
    (e: any) => e.value === item.value && e.map === item.map
  )
...
const useFacetNavigation = (selectedFacets: any, scrollToTop: any) => {
  const { navigate, setQuery, query: runtimeQuery } = useRuntime()
  const { fuzzy, operator, searchState }: any = useSearchState()
